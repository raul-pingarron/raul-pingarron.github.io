...
Para utilizar esta clase y poder acceder a un conjunto de datos que se encuentre en el *LocalFileSystem* se emplea el URI de tipo `file:///` y lo único que tendremos que hacer es asegurarnos de que hay una copia del conjunto de datos en cada nodo worker. Esto último con NFS es muy simple: basta con montar el filesystem remoto en cada uno de los nodos workers sobre el mismo punto de montaje.
